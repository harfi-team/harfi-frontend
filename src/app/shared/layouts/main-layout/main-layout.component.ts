  ...
  constructor() {
    this.router.events.pipe(
      ...
      map(() => {
        ...
        if (url.includes('/admin/dashboard')) return { key: 'ADMIN.DASHBOARD',         admin: true,  ai: false };
        if (url.includes('/admin/craftsmen')) return { key: 'ADMIN.MANAGE_CRAFTSMEN',  admin: true,  ai: false };
        if (url.includes('/admin/customers')) return { key: 'ADMIN.MANAGE_CUSTOMERS',  admin: true,  ai: false };
        if (url.includes('/admin/jobs'))      return { key: 'ADMIN.MANAGE_JOBS',        admin: true,  ai: false };
        if (url.includes('/admin/reviews'))   return { key: 'ADMIN.REVIEWS',            admin: true,  ai: false };
        if (url.includes('/admin/settings'))  return { key: 'ADMIN.SETTINGS',           admin: true,  ai: false };
        ...
        if (url.includes('/notifications'))   return { key: 'NOTIFICATIONS',            admin: false, ai: false };
