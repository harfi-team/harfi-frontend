...
import { catchError } from 'rxjs/operators';
  ...
  private craftsmanSvc: CraftsmanService,   // ← جديد


    ) {}


private enrichCraftsmenWithPhotos(craftsmen: RetrievedCraftsman[] | undefined): void {
  if (!craftsmen || craftsmen.length === 0) return;
  const requests = craftsmen.map(c =>
    this.craftsmanSvc.getCraftsman(String(c.id)).pipe(catchError(() => of(null)))
  );
  forkJoin(requests).subscribe(results => {
    results.forEach((full, i) => {
      if (full?.avatarUrl) craftsmen[i].profileImageUrl = full.avatarUrl;
    });
  ...
  ngOnInit() {
    if (this.languageService.current() !== 'ar') {
      this.languageService.switchTo('ar');
    }
  ...
  ngOnDestroy() {
    // احنا مش بنغير حاجة هنا عشان نحافظ على العربية في الصفحة
    // لكن لو عاوز ترجع للغة السابقة، اعمل الآتي:
    const prevLang = localStorage.getItem('harfi_previous_lang');
    if (prevLang && prevLang !== this.languageService.current()) {
      this.languageService.switchTo(prevLang as 'ar' | 'en');
    }
  }
  ...
  openSession(sessionId: string) {
    ...
    this.enrichCraftsmenWithPhotos(m.craftsmenResult!.craftsmen);
      ...
      if (r.isComplete && r.result) {
        ...
        this.chatMessages.push({
          ...
          detectedCity: r.extractedCity
                ...
                this.enrichCraftsmenWithPhotos(r.result.retrievedCraftsmen);
